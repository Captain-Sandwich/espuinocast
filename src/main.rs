...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let host = match args.address {
        ...
        None => config
            .get("espuino", "host")
            .unwrap_or(String::from("espuino.local")),
    };
    let proxy_url = config.get("espuino", "host");
    let mut directory = config
        .get("espuino", "path")
        .unwrap_or(String::from("/podcasts/"));
    ...
    for section in config
        .get_map_ref()
        .keys()
        .filter(|s| s.starts_with("podcast."))
    {
        ...
        let truncate = config
            ...
            .map(|x| x as usize);
        let reverse = config
            .getboolcoerce(&section, "reverse")
            .unwrap_or(Some(false))
            .unwrap_or(false);
        let to_file = if args.force_write {
            Some(format!("./{}.m3u", name))
        } else {
        ...
        if let Some(fname) = to_file {
            write_m3u(&fname, &playlist)?
        }

        println!(
            "Finished processing podcast \"{}\" ({} elements)",
            name,
            playlist.len()
        );
    ...
    println!("Starting uploads to ESPuino ({})", host);
    ...
    let response = client.get(api_url.clone()).query(&params).send().await?;
    let dirtree: Vec<TreeEntry> = response.json().await?;
    let directory_exists = dirtree
        .iter()
        .any(|item| (item.name == basename) && (item.dir == Some(true)));
    if !directory_exists {
        println!(
            "Directory does not exist. Creating {:?}",
            podcast_path.as_os_str()
        );
        let params = [("path", podcast_path)];
        client.put(api_url.clone()).query(&params).send().await?;
    }
    ...
    for (name, playlist) in playlists {
        ...
        write_playlist_to_server(&client, &api_url, &playlist, path.to_str().unwrap()).await?;
        println!("Wrote {} to ESPuino", name);
    }

    // list podcast directory
    let params = [("path", podcast_path.to_str().unwrap().strip_suffix("/"))];
    let response = client.get(api_url).query(&params).send().await?;
    let dirtree: Vec<TreeEntry> = response.json().await?;
    println!("Podcast directory:\n{:?}", dirtree);
...
async fn write_playlist_to_server(
    client: &reqwest::Client,
    base_url: &Url,
    playlist: &Vec<m3u::Entry>,
    path: &str,
) -> reqwest::Result<()> {
...
async fn process_rss(
    ...
    let mut episodes: Vec<_> = rss
        ...
        .map(|e| {
            ...
            if url.scheme() == "https" {
                // try to change https to http due to ESP limitations
